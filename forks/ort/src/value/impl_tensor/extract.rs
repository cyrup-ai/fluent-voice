	...
	match value.dtype() {
		ValueType::Tensor { ty, shape, .. } => {
			...
			if *ty == expected_ty {
				...
				Err(Error::new_with_code(ErrorCode::InvalidArgument, format!("Cannot extract Tensor<{expected_ty}> from Tensor<{ty}>")))
			}
		}
		t => Err(Error::new_with_code(ErrorCode::InvalidArgument, format!("Cannot extract a Tensor<{expected_ty}> from {t}")))
	}
}
