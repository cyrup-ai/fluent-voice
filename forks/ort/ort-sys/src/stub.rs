...
impl Error {
	...
	/// Cast an ORT status pointer to a borrowed Error reference.
	///
	/// # Safety
	///
	/// The caller must ensure that:
	/// - `status` is a valid, non-null pointer to an OrtStatus created by this library
	/// - The pointer was originally created from a valid Error struct via `into_sys()`
	/// - The Error struct remains valid for the lifetime 'e
	/// - No mutable references to the same Error exist during the lifetime 'e
	pub unsafe fn cast_from_sys<'e>(status: *const OrtStatus) -> &'e Error {
		unsafe { &*status.cast::<Error>() }
	}

	/// Consume an ORT status pointer and convert it back to a boxed Error.
	///
	/// # Safety
	///
	/// The caller must ensure that:
	/// - `status` is a valid, non-null pointer to an OrtStatus created by this library
	/// - The pointer was originally created from a valid Error struct via `into_sys()`
	/// - The pointer has not been previously consumed or freed
	/// - No references to the Error exist when this function is called
	/// - The caller takes ownership of the returned Box and is responsible for its deallocation
