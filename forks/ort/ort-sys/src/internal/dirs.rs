...
mod unix {
	...
	pub fn home_dir() -> Option<PathBuf> {
		...
		unsafe fn fallback() -> Option<OsString> {
			let amt = match unsafe {
				// SAFETY: sysconf is a standard POSIX function
				sysconf(SC_GETPW_R_SIZE_MAX)
			} {
				n if n < 0 => 512,
				n => n as usize
			};
			let mut buf = Vec::with_capacity(amt);
			let mut passwd: passwd = unsafe {
				// SAFETY: passwd is a C struct that can be safely zero-initialized
				mem::zeroed()
			};
			let mut result = ptr::null_mut();
			match unsafe {
				// SAFETY: getpwuid_r is a standard POSIX function, all parameters are valid
				getpwuid_r(getuid(), &mut passwd, buf.as_mut_ptr(), buf.capacity(), &mut result)
			} {
				0 if !result.is_null() => {
					let ptr = passwd.pw_dir as *const _;
					let bytes = unsafe {
						// SAFETY: passwd.pw_dir is a valid C string pointer from getpwuid_r
						CStr::from_ptr(ptr)
					}
					.to_bytes();
