    ...
    (
        ...
        impl fluent_voice_domain::fluent_voice_domain::FluentVoice for $engine {
            ...
            fn stt() -> impl fluent_voice_domain::stt_conversation::SttConversationBuilder {
                // TTS-only engines do not support STT - return proper error at compile time
                compile_error!("TTS-only engine does not support STT operations. Use a dedicated STT engine or a combined TTS/STT engine.")
            }
        }
    };
}
