...
export const asSamples =(ms:number,sr:number)=> Math.round((ms*sr)/1000);
...
function fadeTable(len:number,inOut:"in"|"out",curve:string){
  const k=`${len}|${inOut}|${curve}`; if(FADE_LUT.has(k)) return FADE_LUT.get(k)!;
  const t=new Float32Array(len);
  for(let i=0;i<len;i++){const x=i/len;
    switch(curve){
      case"exponential":t[i]=inOut==="in"?x*x:(1-x)*(1-x);break;
      case"logarithmic":t[i]=inOut==="in"?Math.sqrt(x):Math.sqrt(1-x);break;
      case"sinusoidal":t[i]=inOut==="in"?Math.sin(x*Math.PI/2):
                                       Math.sin((1-x)*Math.PI/2);break;
      default:t[i]=inOut==="in"?x:1-x;
    }}
  FADE_LUT.set(k,t);return t;}
export function applyFade(buf:Float32Array,off:number,len:number,
                          fadeIn:boolean,curve:string){
  if(len<=0)return;const tbl=fadeTable(len,fadeIn?"in":"out",curve);
...
export function estimateBufferSize(
  jitter:number,loss:number,conn:string,sr:number){
  const base=CONSTANTS.CONNECTION_TYPE_BUFFER_SIZES[
