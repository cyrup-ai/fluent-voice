use fluent_voice_vad::{VoiceActivityDetector, Sample};
...
fn test_vad_basic_functionality() {
    ...
    let silence_probability = vad.predict(silence.iter().copied())
        .expect("Failed to predict silence");
    
    assert!(silence_probability < 0.5, 
        "Silence should have low speech probability, got: {}", silence_probability);
    ...
    let speech_probability = vad.predict(speech_signal.iter().copied())
        .expect("Failed to predict speech signal");
    
    // Speech signal should have higher probability than silence
    assert!(speech_probability >= silence_probability,
        "Speech signal ({}) should have higher probability than silence ({})", 
...
fn test_vad_state_reset() {
    ...
    let _prob1 = vad.predict(audio.iter().copied())
    ...
    let _prob2 = vad.predict(audio.iter().copied())
...
fn test_vad_performance_characteristics() {
    ...
    for _ in 0..iterations {
        let _ = vad.predict(audio.iter().copied())
    ...
    assert!(avg_ms < 10.0, "VAD should be fast enough for real-time, got {}ms", avg_ms);
}
