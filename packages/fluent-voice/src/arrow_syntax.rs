...
macro_rules! arrow_syntax {
    // Transform on_chunk calls with arrow syntax
    (
        $builder:expr => on_chunk(|$param:ident| {
            Ok => $ok_expr:expr,
...
pub trait ArrowSyntaxBuilder<T> {
    /// Version of on_chunk that supports arrow syntax
    fn on_chunk_arrow<F, R>(self, f: F) -> Self
    where
        F: ArrowSyntaxClosure<T, R>;
}
