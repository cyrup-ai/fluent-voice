...
pub fn build_mel_filters<T: Float>(cfg: &super::Config) -> Result<Vec<T>, Box<dyn std::error::Error>> {
    let n_mels = cfg.num_mel_bins;
    let n_fft = super::N_FFT;
    let sample_rate = super::SAMPLE_RATE;
    let fmax = T::from(sample_rate).unwrap() / T::from(2).unwrap();
    
    // Convert frequencies to mel scale
    let mel_min = T::from(0.0).unwrap();
    let mel_max = hz_to_mel(fmax);
    
    // Create equally spaced mel frequencies
    let mel_points = (0..=n_mels + 1)
        .map(|i| {
            mel_min + (mel_max - mel_min) * T::from(i).unwrap() / T::from(n_mels + 1).unwrap()
        })
        .collect::<Vec<_>>();
    
    // Convert mel frequencies back to Hz
    let freq_points = mel_points.iter().map(|&mel| mel_to_hz(mel)).collect::<Vec<_>>();
    
    // Create filter bank
    let n_freqs = n_fft / 2 + 1;
    let mut filters = vec![T::zero(); n_mels * n_freqs];
    ...
    for m in 0..n_mels {
        ...
        for k in 0..n_freqs {
            let freq = T::from(k).unwrap() * T::from(sample_rate).unwrap() / T::from(n_fft).unwrap();
...
fn hz_to_mel<T: Float>(hz: T) -> T {
    T::from(2595.0).unwrap() * (T::one() + hz / T::from(700.0).unwrap()).log10()
}

/// Convert mel scale value back to Hz
fn mel_to_hz<T: Float>(mel: T) -> T {
    T::from(700.0).unwrap() * ((mel / T::from(2595.0).unwrap()).exp10() - T::one())
}
