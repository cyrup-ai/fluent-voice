...
use crate::stream_both::{BidirectionalStream, StreamEvent, Config};
use crate::error::MoshiError;
use fluent_voice_livekit::{Room, RoomEvent, RemoteAudioTrack, LocalTrackPublication};
use fluent_voice_livekit::playback::NativeAudioStream;
...
impl LiveKitBidirectionalBridge {
    ...
    pub async fn connect(
        ...
        info!("Connecting LiveKit bidirectional bridge to room: {}", room_url);
        
        // Use existing production LiveKit Room::connect
        let (room, room_events) = Room::connect(room_url.clone(), token).await
    ...
    pub async fn start_microphone(&mut self) -> Result<(), MoshiError> {
        ...
        let (publication, _stream) = self.room.publish_local_microphone_track().await
    ...
    pub async fn stop_microphone(&mut self) -> Result<(), MoshiError> {
        if let Some(publication) = &self.microphone_track {
            ...
            self.room.unpublish_local_track(publication.sid()).await
                .map_err(|e| MoshiError::Custom(format!("Failed to unpublish microphone: {}", e)))?;
        ...
        while let Ok(Some(event)) = self.bidirectional.try_next().await {
            match event? {
                ...
                StreamEvent::UserSpeech { words, is_final } => {
                    if is_final {
                        let transcript: Vec<String> = words.iter()
                ...
                StreamEvent::LatencyUpdate { asr_latency_ms, tts_latency_ms, conversation_latency_ms, timestamp } => {
                    debug!("Latency update - ASR: {:.1}ms, TTS: {:.1}ms, Conversation: {:.1}ms @ {:.3}s", 
                    ...
                    if asr_latency_ms > self.config.max_latency_ms as f64 {
                        warn!("ASR latency exceeded threshold: {:.1}ms > {}ms", 
    ...
    async fn process_audio_channels(&mut self) -> Result<(), MoshiError> {
        ...
        while let Ok(audio_samples) = self.audio_to_kyutai_rx.try_recv() {
            debug!("Processing {} remote audio samples through kyutai", audio_samples.len());
        ...
        while let Ok(kyutai_audio) = self.kyutai_to_livekit_rx.try_recv() {
            debug!("Transmitting {} kyutai response samples to LiveKit room", kyutai_audio.len());
            
            // Convert f32 to i16 for LiveKit transmission
            let livekit_samples: Vec<i16> = kyutai_audio.iter()
                .map(|&sample| (sample.clamp(-1.0, 1.0) * 32767.0) as i16)
                .collect();
            
            // IMPLEMENTED: Actual LiveKit audio transmission
            if let Some(publication) = &self.microphone_track {
                if let Err(e) = self.room.send_audio_frame(publication.sid(), &livekit_samples).await {
                    error!("Failed to transmit audio to LiveKit room: {}", e);
                    return Err(MoshiError::Custom(format!("LiveKit transmission failed: {}", e)));
                }
                debug!("Successfully transmitted {} samples to LiveKit room", livekit_samples.len());
            } else {
        ...
        while let Ok(event) = self.room_events.try_recv() {
            match event {
                RoomEvent::TrackSubscribed { track, participant, .. } => {
                    info!("Track subscribed from participant: {:?}", participant.identity());
                    ...
                    if let fluent_voice_livekit::RemoteTrack::Audio(audio_track) = track {
                        ...
                        self.remote_audio_streams.insert(participant_id.clone(), stream);
                        
                        // Bridge remote audio to kyutai processing
                        self.bridge_remote_audio_to_kyutai(audio_track).await?;
                        info!("Audio bridge established for participant: {}", participant_id);
                    }
                }
    ...
    async fn send_audio_to_room(&self, audio_data: Vec<f32>) -> Result<(), MoshiError> {
        ...
        let livekit_samples: Vec<i16> = audio_data.iter()
            .map(|&sample| (sample.clamp(-1.0, 1.0) * 32767.0) as i16)
            .collect();
        
        debug!("Converted {} samples for LiveKit transmission", livekit_samples.len());
        
        // IMPLEMENTED: Actual transmission to LiveKit room via channel
        if let Err(e) = self.kyutai_to_livekit_tx.send(audio_data.clone()) {
            return Err(MoshiError::Custom(format!("Failed to send audio to LiveKit transmission queue: {}", e)));
        }
        
        debug!("Sent {} samples to LiveKit transmission queue", livekit_samples.len());
