//! Conditioner module for Moshi language model
//! 
//! Provides conditioning functionality for the language model including
//! LUT (lookup table) conditioning and tensor conditioning.
...
impl LutConditioner {
    pub fn new(output_dim: usize, n_bins: usize, dim: usize, possible_values: Vec<String>, vb: VarBuilder) -> Result<Self> {
    ...
    pub fn condition(&self, value: &str, device: &Device) -> Result<Tensor> {
        let idx = *self.possible_values.get(value).ok_or_else(|| candle::Error::msg(format!("unknown value '{}'", value)))?;
...
impl TensorConditioner {
    ...
    pub fn condition(&self, tensor: Tensor) -> Result<Tensor> {
        if tensor.dim(D::Minus1)? != self.dim {
            return Err(candle::Error::msg("dimension mismatch in tensor conditioner"));
        }
        Ok(tensor)
    }
}
...
impl ConditionProvider {
    pub fn new(output_dim: usize, config: &HashMap<String, (String, usize, usize, Vec<String>)>, vb: VarBuilder) -> Result<Self> {
        ...
        for (name, (typ, n_bins, dim, possible_values)) in config {
            ...
            let conditioner = match typ.as_str() {
                "lut" => Conditioner::Lut(LutConditioner::new(output_dim, *n_bins, *dim, possible_values.clone(), conditioner_vb)?),
                "tensor" => Conditioner::Tensor(TensorConditioner::new(*dim)),
                _ => return Err(candle::Error::msg(format!("unknown conditioner type '{}'", typ))),
            };
    ...
    pub fn get_condition(&self, name: &str, value: &str, device: &Device) -> Result<Tensor> {
        match self.conditioners.get(name) {
            ...
            Some(Conditioner::Tensor(tensor)) => tensor.condition(Tensor::zeros((1, 1, tensor.dim), DType::F32, device)?),
            None => Err(candle::Error::msg(format!("unknown conditioner '{}'", name))),
        }
    }
}
