...
impl KyutaiTokenizer {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        let tokenizer = Tokenizer::from_file(path).map_err(|e| {
            MoshiError::Tokenization(format!("Failed to load tokenizer from file: {}", e))
        })?;
    ...
    pub fn from_pretrained(model_name: &str) -> Result<Self> {
        let tokenizer = Tokenizer::from_pretrained(model_name, None).map_err(|e| {
            MoshiError::Tokenization(format!(
    ...
    fn from_tokenizer(tokenizer: Tokenizer) -> Self {
        ...
        let bos_token_id =
            Self::find_special_token_id(&tokenizer, &["<|startoftext|>", "<s>", "[BOS]", "<bos>"]);
        let eos_token_id =
            Self::find_special_token_id(&tokenizer, &["<|endoftext|>", "</s>", "[EOS]", "<eos>"]);
    ...
    pub fn build(self) -> Result<KyutaiTokenizer> {
        let mut tokenizer = if let Some(path) = self.tokenizer_path {
            ...
            return Err(MoshiError::Tokenization(
                "Pretrained models require 'http' feature".to_string(),
            ));
        } else {
            return Err(MoshiError::Tokenization(
                "Must specify either file path or pretrained model".to_string(),
            ));
        };

        // Override special tokens if provided
        if let Some(id) = self.custom_bos {
            tokenizer.set_bos_token_id(id);
        }
        if let Some(id) = self.custom_eos {
            tokenizer.set_eos_token_id(id);
        }
        if let Some(id) = self.custom_pad {
            tokenizer.set_pad_token_id(id);
        }
        if let Some(id) = self.custom_unk {
            tokenizer.set_unk_token_id(id);
        }
