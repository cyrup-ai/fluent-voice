...
use crate::{
    config::Config,
    conditioner::{ConditionProvider, Condition},
    streaming::{StreamingTransformer, CaSrc},
    transformer,
    error::MoshiError,
};
...
impl LmModel {
    ...
    pub fn new(config: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_tokens = candle_nn::embedding(config.vocab_size, config.d_model, vb.pp("embed_tokens"))?;
        ...
        let output_proj = candle_nn::linear(config.d_model, config.vocab_size, vb.pp("output_proj"))?;
        ...
        let condition_provider = if !config.conditioning.lut_conditioners.is_empty() || 
            ...
            for (name, lut_config) in &config.conditioning.lut_conditioners {
                conditioner_config.insert(
                    name.clone(),
                    ("lut".to_string(), lut_config.n_bins, lut_config.dim, lut_config.possible_values.clone())
                );
            }
            ...
            Some(ConditionProvider::new(output_dim, &conditioner_config, vb.pp("condition_provider"))?)
        } else {
            None
        };
    ...
    pub fn forward(&mut self, input_ids: &Tensor, conditions: Option<&[Condition]>) -> Result<Tensor> {
        // Embed input tokens
    ...
    pub fn forward_with_ca(&mut self, input_ids: &Tensor, ca_src: Option<&CaSrc>, conditions: Option<&[Condition]>) -> Result<Tensor> {
        // Embed input tokens
        let mut hidden_states = input_ids.apply(&self.embed_tokens)?;
    ...
    pub fn generate(&mut self, 
                   ...
                   temperature: f64, 
                   top_k: Option<usize>, 
                   conditions: Option<&[Condition]>) -> Result<Tensor> {
    ...
    pub fn forward(&mut self, text: Option<Tensor>, audio_tokens: Vec<Option<Tensor>>) -> Result<(Tensor, Tensor)> {
        ...
        let text_input = if let Some(text) = text {
            ...
            Tensor::from_vec(vec![self.text_start_token()], (batch_size, seq_len), &self.device)?
        };
        ...
        if !audio_tokens.is_empty() {
            ...
            let audio_condition = Tensor::zeros((batch_size, seq_len, audio_dim), hidden_states.dtype(), &self.device)?;
    ...
    fn top_k_filter(&self, logits: &Tensor, k: usize) -> Result<Tensor> {
        ...
        let (top_k_values, top_k_indices) = logits.topk(k)?;
        
        // Create mask for top-k
        let mut filtered = Tensor::full(f32::NEG_INFINITY, logits.shape(), logits.device())?;
        
        // Set top-k values
        for batch_idx in 0..logits.dim(0)? {
            for i in 0..k {
                let idx = top_k_indices.get(batch_idx)?.get(i)?.to_scalar::<u32>()? as usize;
                let value = top_k_values.get(batch_idx)?.get(i)?.to_scalar::<f32>()?;
                filtered = filtered.slice_assign(&[batch_idx..batch_idx+1, idx..idx+1], &Tensor::full(value, (1, 1), logits.device())?)?;
            }
        }
    ...
    fn sample_from_probs(&self, probs: &Tensor) -> Result<Tensor> {
        ...
        let (_values, indices) = probs.argmax_keepdim(D::Minus1)?;
        Ok(indices)
    }
}
...
impl LmModelBuilder {
    ...
    pub fn build(self, vb: VarBuilder) -> Result<LmModel> {
        ...
        self.config.validate().map_err(|e| candle::Error::msg(format!("Config validation failed: {}", e)))?;
