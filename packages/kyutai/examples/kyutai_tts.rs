...
use fluent_voice_domain::{PitchRange, VocalSpeedMod};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut audio_stream = KyutaiEngine::tts()
        .conversation()
        .with_speaker(KyutaiSpeakerLine {
        ...
        .synthesize(|result| {
            ...
            let stream: Pin<
                Box<dyn Stream<Item = fluent_voice_domain::audio_chunk::AudioChunk> + Send + Unpin>,
            > = match result {
