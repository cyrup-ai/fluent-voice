...
use fluent_voice_elevenlabs::audio_format_detection::{
    AudioFormatDetector, AudioMetadata, AudioParams, ConfidenceLevel, DetectedFormat,
    DetectionLayer, FormatDetectionResult,
};
...
mod audio_format_detection_tests {
    ...
    fn test_multiple_content_types() {
        ...
        for (content_type, should_succeed) in test_cases {
            ...
            if should_succeed {
                assert!(
                    result.is_ok(),
                    "Should succeed for content-type: {}",
                    content_type
                );
            } else {
    ...
    fn test_invalid_query_parameters() {
        ...
        query
            .params
            .push(("output_format", "invalid_format".to_string()));
    ...
    fn test_detection_performance() {
        ...
        assert!(
            ...
            "Detection too slow: {}ms average",
            avg_time.as_millis()
        );

        println!(
            "Performance: {} detections in {:?} (avg: {:?})",
