...
use fluent_voice_domain::{
    Language, SpeechSource, TranscriptionSegmentImpl, VoiceError, WordTimestamps,
};
...
impl TtsConversationBuilder {
    ...
    pub async fn synthesize<M, S>(self, matcher: M) -> std::result::Result<S, Box<dyn std::error::Error + Send + Sync>>
    where
        ...
        S: futures_util::Stream<Item = i16> + Send + Unpin + 'static,
    {
        ...
        let audio_stream = tts_builder.stream().await?;
...
impl TtsConversation {
    pub fn into_stream(self) -> impl futures_util::Stream<Item = i16> + Send + Unpin {
        // Convert AudioStream (bytes) to i16 stream
        // This is a simplified conversion - in real implementation we'd properly decode audio
        self.audio_stream.map(|bytes_result| {
            match bytes_result {
                Ok(bytes) => {
                    // Convert first byte to i16, or 0 if empty
                    bytes.get(0).map(|&b| b as i16).unwrap_or(0)
                }
                Err(_) => 0i16, // Return silence on error
            }
        })
    }
}
...
impl AudioChunk {
    ...
    pub fn bad_chunk(_error: String) -> Self {
        Self {
            data: Vec::new(),
        }
    }
...
impl TtsBuilder {
    ...
    {
        let _engine = self.engine_builder.build()?;
