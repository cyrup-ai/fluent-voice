...
impl SttBuilder {
    ...
    /// Listen to microphone (placeholder - ElevenLabs doesn't support live STT yet)
    pub fn microphone(self, _device: impl Into<String>) -> MicrophoneBuilder {
        MicrophoneBuilder { builder: self }
    }
...
/// Microphone transcription builder (placeholder)
pub struct MicrophoneBuilder {
    builder: SttBuilder,
}

impl MicrophoneBuilder {
    /// Start listening (not implemented - ElevenLabs doesn't support live STT)
    pub async fn listen(self) -> Result<TranscriptStream> {
        Err("Live microphone transcription is not supported by ElevenLabs API".into())
    }
}
...
/// Transcript stream (placeholder for live transcription)
