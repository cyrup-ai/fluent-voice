...
impl ApiSchema {
    ...
    pub fn with_path_params<const N: usize>(
        mut self,
        path_params_schema: [(&str, &str); N],
    ) -> Self {
        let map = path_params_schema
            .into_iter()
            .map(|(k, _v)| (k.to_string(), ParamSchema::new_string("Default parameter"))) // Default ParamSchema
            .collect::<HashMap<String, ParamSchema>>();
        self.path_params_schema = Some(map);
        self
    }
