...
fn main() -> anyhow::Result<()> {
    ...
    println!("   Confidence threshold: {}", wake_unwake_config.confidence_threshold);
    println!();
    ...
    println!("🎯 Say '{}' to wake up, '{}' to go back to sleep", 
        detector.get_current_state(), "syrup stop");
                ...
                move |data: &[f32], _: &cpal::InputCallbackInfo| {
                    ...
                    match detector.process_samples(data) {
                        Ok(Some(detection)) => {
                            ...
                            if detection.phrase.contains("syrup") && !detection.phrase.contains("stop") {
                                wake_detections_cb.fetch_add(1, Ordering::Relaxed);
                                println!("🌅 WAKE Detection #{}: '{}' (confidence: {:.3}) - State: {}", 
                                ...
                                println!("😴 UNWAKE Detection #{}: '{}' (confidence: {:.3}) - State: {}", 
                                    detection_count, detection.phrase, detection.confidence, current_state);
                    ...
                    if chunk_count % 2000 == 0 {
                        ...
                        println!("📊 Status: {} chunks processed | State: {} | Wake: {} | Unwake: {}", 
