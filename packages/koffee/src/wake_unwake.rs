...
impl WakeUnwakeConfig {
    ...
    pub fn validate(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if !std::path::Path::new(&self.wake_model_path).exists() {
            return Err(format!(
                "Wake model file does not exist: {}",
        ...
        std::fs::File::open(&self.wake_model_path)
            .map_err(|e| format!("Cannot read wake model file '{}': {}", self.wake_model_path, e))?;

        std::fs::File::open(&self.unwake_model_path)
            .map_err(|e| format!("Cannot read unwake model file '{}': {}", self.unwake_model_path, e))?;
...
impl DetectorMetrics {
    ...
    pub fn update_processing_time(&self, duration: Duration) {
        ...
        self.total_processing_time_us.fetch_add(duration_us, Ordering::Relaxed);
        ...
        self.avg_processing_time_us.store(total_time / iterations, Ordering::Relaxed);
    }
...
impl WakeUnwakeDetector {
    ...
    pub fn new(
        ...
        let current_state = Arc::new(std::sync::atomic::AtomicU8::new(WakeUnwakeState::Sleep as u8));
    ...
    pub fn process_samples(&self, samples: Vec<f32>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub fn recv_event_timeout(&self, timeout: Duration) -> Result<DetectorEvent, crossbeam_channel::RecvTimeoutError> {
                    ...
                    match command {
                        ...
                        DetectorCommand::SwapToWakeModel => {
                            match Self::load_wake_model(&config, &detector_config) {
                                Ok(new_detector) => {
                                    ...
                                    current_state.store(WakeUnwakeState::Sleep as u8, Ordering::Release);
                                    metrics.increment_model_swaps();
                                    let swap_duration = start_time.elapsed();
                                    
                                    let _ = event_sender.try_send(DetectorEvent::ModelSwapCompleted {
                        ...
                        DetectorCommand::SwapToUnwakeModel => {
                            match Self::load_unwake_model(&config, &detector_config) {
                                Ok(new_detector) => {
                                    current_detector = new_detector;
                                    current_state.store(WakeUnwakeState::Awake as u8, Ordering::Release);
                                    metrics.increment_model_swaps();
                                    let swap_duration = start_time.elapsed();
    ...
    fn load_wake_model(
        ...
        let model_data = std::fs::read(&config.wake_model_path)
            .map_err(|e| format!("Failed to read wake model file '{}': {}", config.wake_model_path, e))?;
        
        let model = WakewordModel::load_from_buffer(&model_data)
            .map_err(|e| format!("Failed to load wake model: {}", e))?;
        
        KoffeeCandle::new(model, detector_config.clone())
            .map_err(|e| format!("Failed to create wake detector: {}", e).into())
    }
    ...
    fn load_unwake_model(
        ...
        let model_data = std::fs::read(&config.unwake_model_path)
            .map_err(|e| format!("Failed to read unwake model file '{}': {}", config.unwake_model_path, e))?;
        
        let model = WakewordModel::load_from_buffer(&model_data)
            .map_err(|e| format!("Failed to load unwake model: {}", e))?;
        
        KoffeeCandle::new(model, detector_config.clone())
            .map_err(|e| format!("Failed to create unwake detector: {}", e).into())
    }
