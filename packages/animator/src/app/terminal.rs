...
use crate::{
    ...
    oscillator::{
        Dimension, DisplayMode, GraphConfig, UIMode, update_value_f, update_value_i, vector::Vector,
    },
};
...
impl App {
    ...
    pub fn run<T: Backend>(
        ...
        loop {
            let data = source.recv().map_err(|e| {
                io::Error::new(
                    io::ErrorKind::BrokenPipe,
                    format!("Audio source error: {}", e),
                )
            })?;
...

            {
                ...
                if let Err(e) = terminal.draw(|f| {
                    ...
                    let ui_mode = UIMode::from_show_ui(self.graph.show_ui);
                    let chart = Chart::new(datasets.iter().map(|x| x.into()).collect())
                        .x_axis(
                            self.current_display()
                                .axis(&self.graph, ui_mode, Dimension::X),
                        )
                        .y_axis(
                            self.current_display()
                                .axis(&self.graph, ui_mode, Dimension::Y),
                        );
