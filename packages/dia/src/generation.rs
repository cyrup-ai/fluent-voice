...
fn topk_topp_mask(logits: &Tensor, top_k: usize, top_p: f64) -> Result<Tensor> {
    ...
    if (0.0..1.0).contains(&top_p) {
        ...
        indices.sort_by(|&a, &b| pvec[b].partial_cmp(&pvec[a]).unwrap()); // descending
...
impl DiaTts {
    pub fn new(model: DiaModel, cfg: DiaConfig, device: Device) -> Self {
        ...
        if matches!(device, Device::Cuda(_) | Device::Metal(_)) {
            ...
            println!("  - Mixed precision: {}", gpu_config.mixed_precision);
            println!("  - Compute dtype: {:?}", compute_dtype);
            println!("  - Optimal batch size: {}", gpu_config.optimal_batch_size);
        }
    ...
    fn decoder_step(
        ...
        let guided = (&uncond + ((&cond - &uncond)? * cfg_scale)?)?; // [C,V]
        ...
        for c in 0..guided.dim(0)? {
            ...
            logit = topk_topp_mask(&logit, top_k, top_p)?;
    ...
    pub fn generate_speech(
        ...
        if let Some(prompt) = audio_prompt {
            ...
            let bos_row = Tensor::full(bos as u32, (1, self.cfg.data.channels), &self.device)?;
            ...
            let bos_tok = Tensor::full(bos as u32, (1, 1, self.cfg.data.channels), &self.device)?; // [1,1,C]
        ...
        while cur < max_tokens && cur < self.cfg.data.audio_length {
            dec_state.prepare_step(cur);

            // Last generated token (or BOS at cur==0) duplicated for CFG.
            let prev = if cur == 0 {
                Tensor::full(bos as u32, (1, 1, self.cfg.data.channels), &self.device)?
            } else {
