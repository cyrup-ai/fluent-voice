...
impl<S: Speaker> Conversation<S> {
    ...
    pub fn new(text: String, speaker: S, pool: Arc<VoicePool>) -> Result<Self> {
        Ok(Self {
            text,
            speaker,
            pool,
    ...
    pub async fn internal_generate(&self) -> Result<VoicePlayer, VoiceError> {
        ...
        let prompt_b2s = prompt_ids
            .expand(&[2, prompt_ids.dim(1).unwrap()])
        ...
        if let Some(ap) = &audio_prompt_codes {
            ...
            let ap_b2tc = ap_delayed
                ...
                .expand(&[2, ap.dim(0).unwrap(), ap.dim(1).unwrap()])
                .map_err(|e| {
                    VoiceError::GenerationError(format!("Audio prompt expansion failed: {}", e))
                })?;
            dia.prefill_decoder(&ap_b2tc, &mut dec_state).map_err(|e| {
                VoiceError::GenerationError(format!("Decoder prefill failed: {}", e))
            })?;
        }
