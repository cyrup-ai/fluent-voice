...
/// Video Call CLI
///
/// This cli connects to the llllll-vcall and streams audio and video to the specified meeting.
///
/// You can watch the video at https://llllll-vcall/meeting/{user_id}/{meeting_id}
#[derive(Parser, Debug)]
#[clap(name = "client")]
pub struct Opt {
    #[clap(subcommand)]
...
fn parse_frame_format(s: &str) -> Result<FrameFormat, String> {
    match s {
        ...
        _ => Err("Invalid frame format, please use one of [NV12, YUYV]".to_string()),
    }
}
