...
use rustpotter::{
    Kfc, KkfcAudioFmt, KoffeeCandleConfig, SampleFormat, ScoreMode, VADMode, WakewordLoad,
    WakewordModelTrain, WakewordModelTrainOptions, WakewordRef,
};
...
fn run_bytes(
    ...
    model: &str,
    bytes: Vec<u8>,
) -> Vec<rustpotter::KoffeeCandleDetection> {
...
fn run_samples(
    ...
    cfg.fmt = r.spec().try_into::<AudioFmt>().unwrap();
