...
impl WakewordNN {
    ...
    pub fn new(model: &WakewordModel, score_ref: f32) -> Result<Self, WakewordError> {
        ...
        let model_size = model.train_size * model.kfc_size.0 as usize;
    ...
    fn predict_internal(
        ...
        *scratch = Tensor::from_vec(flat, scratch.dims()[1], &Device::Cpu)?;
        let logits = self.model.forward(&scratch.unsqueeze(0)?)?;
...
const fn inter(inp: usize, kfc: usize, div: usize) -> usize {
    (inp / kfc) / (KFCS_EXTRACTOR_OUT_SHIFTS * div)
}
