...
export async function createSharedRingNode(
...
): Promise<{ node: AudioWorkletNode; enqueue: RingWriter["enqueue"]; sab: SharedArrayBuffer }> {
  /* defaults */
  const ringSeconds = opts.ringSeconds ?? 0.5;
  const sr = opts.sampleRate ?? ctx.sampleRate;
  const url = opts.url ?? "universal-audio.js";
  const channelCount = opts.channelCount ?? 1;

  /* 1. Load module once */
  if (!ctx.audioWorklet.modules.has(url))
    await ctx.audioWorklet.addModule(url);

  /* 2. SAB allocation */
  const capSamples = Math.ceil(sr * ringSeconds);
  ...
  const node = new AudioWorkletNode(ctx, "universal-audio-processor", {
    ...
    processorOptions: { ringBytes: sab, sampleRate: sr }
  });
  ...
  /* 4. writer */
  const enqueue: RingWriter["enqueue"] = pcm => ring.push(pcm);

  return { node, enqueue, sab };
}
